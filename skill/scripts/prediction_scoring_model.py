...
# Knockout policy defaults (overridden by model-hyperparameters.json)
KNOCKOUT_POLICY: dict = {
    "no_primary_draw_unless_coinflip": True,
    "prefer_both_score_primary_in_ko": True,
    "min_scoreline_branches": 3,
    "split_result_score_confidence": True,
    "use_ranking_squad_for_direction": True,
}
...
def load_hyperparameters(root: Path, edition: str) -> None:
    ...
    if path.exists():
        try:
            ...
            policy = data.get("knockout_policy")
            if isinstance(policy, dict) and policy:
                KNOCKOUT_POLICY = {**KNOCKOUT_POLICY, **policy}
...
def _reconcile_evidence_from_disk(
    ...
    for item in items:
        ...
        if eid == "fifa_rankings":
            data = _j_any("rankings/fifa-men-ranking.json", "raw/rankings/fifa-men-ranking.json")
        ...
        # --- historical_worldcup_results ---
            ...
            data = _j_any("history/team-wc-history.json", "raw/history/team-wc-history.json")
        ...
        # --- official_rosters / fixtures ---
        elif eid == "official_rosters":
            data = _j("roster-compiled.json")
            teams = data.get("teams", []) if isinstance(data, dict) else []
            if len(teams) >= 30:
                _upgrade(item, "complete", {"teams": len(teams)}, ["roster_not_compiled"])

        elif eid == "official_fixtures":
            ledger = load_json(ed_root / "match-ledger.json", {})
            matches = ledger.get("matches", [])
            with_ko = sum(1 for m in matches if m.get("kickoff_at"))
            if with_ko >= 40:
                _upgrade(item, "complete", {"matches": len(matches), "matches_with_kickoff": with_ko}, ["fixtures_missing"])
            elif with_ko >= 10:
                _upgrade(item, "partial", {"matches": len(matches), "matches_with_kickoff": with_ko}, ["fixtures_missing"])

        # --- venue_rest_travel ---
        elif eid == "venue_rest_travel":
            rest = _j("evidence/rest-travel-features.json")
            ...
            if len(rest_items) >= 20:
                _upgrade(
                    item, "partial",
                    {"rest_items": len(rest_items), "matches_with_kickoff": with_ko},
                    [
                        "fixture_schedule_required_for_rest_travel",
                        "fixture_schedule_not_imported",
                        "rest_travel_features_not_compiled",
                    ],
                )
            elif with_ko >= 10:
                _upgrade(
        ...
        # --- recent_form_results ---
        elif eid == "recent_form_results":
            data = _j("evidence/recent-form.json")
            form_matches = data.get("matches") or []
            if len(form_matches) >= 20:
                _upgrade(
                    item, "complete",
                    {"matches": len(form_matches), "teams": len(data.get("teams") or [])},
                    ["recent_form_results_missing", "matches_empty"],
                )
            elif form_matches:
                _upgrade(
                    item, "partial",
                    {"matches": len(form_matches)},
                    ["recent_form_results_missing"],
                )

        # --- injury_availability ---
        elif eid == "injury_availability":
            data = _j("evidence/injury-availability.json")
            items_list = data.get("items") or []
            if items_list:
                _upgrade(
                    item, "complete",
                    {"items": len(items_list)},
                    ["daily_injury_availability_check_missing", "items_empty"],
                )

        # --- head_to_head (optional; upgrade if present) ---
        elif eid == "head_to_head":
            data = _j("evidence/head-to-head.json")
            h2h_items = data.get("items") or []
            if h2h_items:
                _upgrade(
                    item, "partial",
                    {"items": len(h2h_items)},
                    ["head_to_head_dataset_missing"],
                )
...
def _determine_confidence(data_score: float, evidence_gaps: list[str]) -> tuple[str, str]:
    """Legacy 2-arg entry: map average data score + gaps into a coarse level.

    Prefer `_score_prediction_confidence` for full multi-factor scoring.
    Critical evidence blocked still demotes; optional gaps only soft-penalize.
    """
    ...
    critical_hard = any(
        any(g.startswith(f"{p}_") for p in critical_prefixes)
        and (g.endswith("_blocked") or g.endswith("_missing"))
        for g in evidence_gaps
    )
    optional_hard = sum(1 for g in evidence_gaps if g.endswith("_blocked") or g.endswith("_missing"))
    optional_partial = sum(1 for g in evidence_gaps if g.endswith("_partial"))

    score = float(data_score)
    if critical_hard:
        score -= 22.0
    score -= min(18.0, optional_hard * 3.5)
    score -= min(10.0, optional_partial * 2.0)
...
def _score_prediction_confidence(
    *,
    data_home: float,
    data_away: float,
    abs_edge: float,
    ...
    dual_track_alignment: str,
    market_status: str,
    predicted_outcome: str,
    phase: str = "",
    three_track_votes: dict | None = None,
) -> dict:
    """Multi-factor confidence scoring — levels are earned, never forced.

    Returns dict with confidence / label / numeric score / factors / split fields.
    High requires several independent supports; a single weak signal cannot mint high.
    """
    labels = {"high": "高信心", "medium": "中等信心", "low": "低信心"}
    factors: list[str] = []
    score = 50.0  # neutral prior

    avg_data = (float(data_home) + float(data_away)) / 2.0
    # 1) Data readiness (both sides have usable fundamentals)
    if avg_data >= 70:
        score += 14.0
        factors.append("data_strong")
    elif avg_data >= 58:
        score += 8.0
        factors.append("data_ok")
    elif avg_data >= 48:
        score += 2.0
        factors.append("data_thin")
    else:
        score -= 12.0
        factors.append("data_weak")

    # 2) Edge magnitude — coinflip cannot be high
    edge = float(abs_edge)
    tier = (edge_tier or _edge_tier(edge)).lower()
    if tier == "strong":
        score += 16.0
        factors.append("edge_strong")
    elif tier == "clear":
        score += 11.0
        factors.append("edge_clear")
    elif tier == "slight":
        score += 4.0
        factors.append("edge_slight")
    else:
        score -= 8.0
        factors.append("edge_coinflip")

    # 3) Evidence quality + structured gaps
    critical_prefixes = (
        "official_fixtures",
        "official_rosters",
        "fifa_rankings",
    ...
    critical_hard = [
        g for g in gaps
        if any(g.startswith(f"{p}_") for p in critical_prefixes)
        and (g.endswith("_blocked") or g.endswith("_missing"))
    ]
    optional_hard = [
        g for g in gaps
        if g not in critical_hard and (g.endswith("_blocked") or g.endswith("_missing"))
    ]
    partials = [g for g in gaps if g.endswith("_partial")]

    eq = (evidence_quality or "thin").lower()
    if eq == "unusable":
        score -= 28.0
        factors.append("evidence_unusable")
    elif eq == "suspect":
        score -= 10.0
        factors.append("evidence_suspect")
    elif eq == "trusted":
        score += 10.0
        factors.append("evidence_trusted")
    else:
        score += 2.0
        factors.append("evidence_thin")

    if critical_hard:
        score -= 18.0 + 4.0 * (len(critical_hard) - 1)
        factors.append(f"critical_gaps:{len(critical_hard)}")
    if optional_hard:
        score -= min(14.0, 2.5 * len(optional_hard))
        factors.append(f"optional_gaps:{len(optional_hard)}")
    if partials:
        score -= min(8.0, 1.5 * len(partials))
        factors.append(f"partial_gaps:{len(partials)}")
    if not gaps:
        score += 6.0
        factors.append("evidence_complete")

    # 4) Scoreline concentration (result direction support)
    top_prob = float(scoreline_distribution[0]["probability"]) if scoreline_distribution else 0.0
    if top_prob >= 0.42:
        score += 8.0
        factors.append("mode_concentrated")
    elif top_prob >= 0.32:
        score += 4.0
        factors.append("mode_ok")
    elif top_prob > 0:
        score -= 4.0
        factors.append("mode_flat")

    # 5) Market dual-track
    align = (dual_track_alignment or "untracked").lower()
    mstat = (market_status or "none").lower()
    if mstat == "trusted_market" and align == "aligned":
        score += 8.0
        factors.append("market_aligned")
    elif mstat == "trusted_market" and align == "divergent":
        score -= 10.0
        factors.append("market_divergent")
    elif mstat == "suspect_market":
        score -= 4.0
        factors.append("market_suspect")
    elif mstat == "mock_invalid":
        score -= 6.0
        factors.append("market_mock")
    else:
        factors.append("market_untracked")

    # 6) Three-track vote agreement (if available)
    votes = three_track_votes or {}
    vote_vals = [str(v).lower() for v in votes.values() if v not in (None, "", "?")]
    if len(vote_vals) >= 2:
        # majority for predicted outcome
        outcome = (predicted_outcome or "").lower()
        agree = sum(1 for v in vote_vals if outcome and (outcome in v or v in outcome))
        if agree >= 2:
            score += 5.0
            factors.append("tracks_agree")
        elif agree == 0 and len(set(vote_vals)) >= 2:
            score -= 4.0
            factors.append("tracks_split")

    # 7) Phase prior: late KO with clear edge is more informative; coinflip KO more volatile
    phase_l = (phase or "").lower()
    if phase_l in {"semi_final", "final", "third_place", "quarter_final"}:
        if tier in {"clear", "strong"}:
            score += 3.0
            factors.append("ko_clear_edge_bonus")
        elif tier == "coinflip":
            score -= 5.0
            factors.append("ko_coinflip_penalty")

    score = max(0.0, min(100.0, score))

    # Hard vetoes
    hard_block_high = bool(critical_hard) or eq == "unusable" or tier == "coinflip"
    hard_block_medium = eq == "unusable" and bool(critical_hard)
    ...
    if hard_block_medium or score < 45.0:
        level = "low"
    elif score >= 72.0 and not hard_block_high and positives >= 2:
        level = "high"
    elif score >= 50.0 or (tier == "coinflip" and positives >= 2 and score >= 46.0):
        # Coinflip can be medium when data + mode support exist; never high.
        level = "medium"
    else:
        level = "low"

    if level == "high" and (hard_block_high or positives < 2):
        level = "medium" if score >= 50.0 else "low"
        factors.append("high_requires_multi_support")

    # Score confidence is strictly harder than result confidence
    if level == "high" and top_prob >= 0.40 and not critical_hard and eq != "unusable":
        score_conf = "high"
    elif level in {"high", "medium"} and top_prob >= 0.30 and not critical_hard:
        score_conf = "medium"
    else:
        score_conf = "low"

    if level == "high" and top_prob >= 0.34:
        goals_conf = "medium" if top_prob < 0.45 else "high"
    elif level == "low":
        goals_conf = "low"
    else:
        goals_conf = "medium" if top_prob >= 0.28 else "low"

    note_parts = [
        f"score={score:.1f}",
        f"edge={tier}",
        f"top_mode={top_prob:.0%}" if top_prob else "top_mode=n/a",
        f"eq={eq}",
    ]
    if critical_hard:
        note_parts.append(f"critical_gaps={len(critical_hard)}")
    note = "认真评分: " + ", ".join(note_parts) + f" → result={level}, score={score_conf}"

    return {
        "confidence": level,
        "confidence_label": labels[level],
        "confidence_score": round(score, 1),
        "confidence_factors": factors,
        "result_confidence": level,
        "score_confidence": score_conf,
        "total_goals_confidence": goals_conf,
        "confidence_note": note,
        "hard_block_high": hard_block_high,
    }
...
def _classify_market_odds(odds: dict | None) -> dict:
    ...
    status_hint = str(odds.get("status") or "").lower()
    # Explicit unavailable / missing feed is not a mock market — treat as no market signal.
    if status_hint in {"unavailable", "missing", "none"} or source in {"odds_unavailable", "missing", "none"}:
        if not odds.get("is_mock") and source != "mock_bookmaker":
            return {
                "status": "none",
                "reason": source or status_hint or "odds_unavailable",
                "is_mock": False,
                "implied_probabilities": None,
                "market_outcome": None,
            }
    if odds.get("is_mock") or source in {"mock_bookmaker"}:
        return {
            "status": "mock_invalid",
...
def _overall_evidence_quality(*, market_status: str, local_gaps: list[str], evidence_gaps: list[str]) -> str:
    """Honest evidence quality used by confidence scoring and scoreline tempering.

    - unusable: mock / invalid market that would poison calibration
    - suspect: market shape outliers
    - trusted: core families present AND no optional family hard-missing
    - thin: missing core data or optional families still blocked/missing
    """
    ...
    all_gaps = list(local_gaps or []) + list(evidence_gaps or [])
    # Operational soft gaps: no live odds/referee is normal pre-kickoff, not thin evidence.
    soft_local = {"odds_missing", "referee_missing"}
    critical_prefixes = ("official_fixtures", "official_rosters", "fifa_rankings", "squad_depth_position_balance")
    critical_hard = [
        g for g in all_gaps
        if any(g.startswith(f"{p}_") for p in critical_prefixes)
        and (g.endswith("_blocked") or g.endswith("_missing"))
    ...
    optional_hard = [
        g for g in all_gaps
        if g not in critical_hard
        and g not in soft_local
        and (g.endswith("_blocked") or g.endswith("_missing"))
    ]
    optional_partial = [g for g in all_gaps if g.endswith("_partial")]

    if critical_hard:
        return "thin"
    if market_status == "trusted_market" and len(optional_hard) <= 2:
        return "trusted"
    if not optional_hard:
        # Core complete; only partial optional families and/or soft local gaps remain.
        if market_status == "trusted_market" or len(optional_partial) <= 3:
            return "trusted"
        return "thin"
    if len(optional_hard) <= 2 and market_status == "trusted_market":
        return "thin"
    return "thin"
...
def _determine_outcome_from_context(
    *,
    ...
    """Directional outcome from multi-signal context — draws are earned, not default.

    Knockout policy (R16 lesson): do not emit primary DRAW on near-parity unless
    signals truly collapse to a coin-flip with no directional lean. Ranking +
    squad depth can break ties when final score is nearly flat.
    """
    gap = home_final - away_final
    abs_gap = abs(gap)
    tier = _edge_tier(abs_gap)
    ranking_gap = float(rs_home) - float(rs_away)
    squad_gap = float(sd_home) - float(sd_away)
    matchup_gap = squad_gap
    # Prefer blended direction: final gap first, then ranking+squad lean.
    blended = gap + 0.35 * ranking_gap + 0.25 * squad_gap
    preferred = "home_win" if blended > 0 else "away_win" if blended < 0 else "draw"
    if preferred == "draw":
        preferred = "home_win" if ranking_gap > 0 else "away_win" if ranking_gap < 0 else "draw"
    ...
    if preferred != "draw" and abs(ranking_gap) >= 1.5:
        alignment_flags.append("ranking")
    three_track_consensus = (three_track_votes or {}).get("consensus")
    three_track_counts = (three_track_votes or {}).get("counts") or {}
    is_ko = _is_knockout_phase(phase)
    draw_trigger = 4.0 if phase == "group" else 2.4
    policy = KNOCKOUT_POLICY if isinstance(KNOCKOUT_POLICY, dict) else {}
    no_primary_draw = bool(policy.get("no_primary_draw_unless_coinflip", True))

    def _directional_from_signals() -> str | None:
        if preferred != "draw":
            return preferred
        if squad_side != "draw":
            return squad_side
        if three_track_consensus and three_track_consensus != "draw":
            return three_track_consensus
        if market_outcome and market_status == "trusted_market" and market_outcome != "draw":
            return market_outcome
        return None

    if tier == "coinflip":
        # Knockout: slight ranking/squad lean is enough for a directional primary.
        if is_ko and no_primary_draw:
            direction = _directional_from_signals()
            if direction and (abs(ranking_gap) >= 0.8 or abs(squad_gap) >= 1.0 or abs_gap >= 0.4 or len(alignment_flags) >= 1):
                return direction, tier, alignment_flags + (["ko_directional_lean"] if "ko_directional_lean" not in alignment_flags else [])
            if direction:
                return direction, tier, alignment_flags
        ...
        # Group-stage coinflip may still draw; KO only if no lean at all.
        if is_ko and no_primary_draw:
            direction = _directional_from_signals()
            if direction:
                return direction, tier, alignment_flags
    ...
    if abs_gap < draw_trigger:
        if len(alignment_flags) >= 1 or (three_track_consensus and three_track_consensus != "draw" and three_track_counts.get(three_track_consensus, 0) >= 2):
            return preferred if preferred != "draw" else (_directional_from_signals() or "draw"), tier, alignment_flags
        if is_ko and no_primary_draw:
            direction = _directional_from_signals()
            if direction:
                return direction, tier, alignment_flags
    ...
    if tier == "slight":
        # Slight edge should stay directional; draw only if all signals collapse.
        if preferred != "draw":
            return preferred, tier, alignment_flags
        if is_ko and no_primary_draw:
            direction = _directional_from_signals()
    ...
    if phase == "group" and evidence_quality in {"thin", "suspect", "unusable"}:
        ...
        if market_status in {"none", "mock_invalid"} and tier == "clear":
            ...
            if 8.0 <= abs_gap < 10.0 and three_track_consensus == preferred and three_track_counts.get(preferred, 0) >= 2:
                return preferred, tier, alignment_flags
            if abs(matchup_gap) <= 7 and abs(rs_home - rs_away) <= 16 and not is_ko:
                return "draw", tier, alignment_flags
        if market_status == "mock_invalid" and abs(matchup_gap) <= 3 and three_track_counts.get(preferred, 0) <= 1 and not is_ko:
            return "draw", tier, alignment_flags
    return preferred if preferred != "draw" else (_directional_from_signals() or "draw"), tier, alignment_flags
...
def _score_pool_for_script(game_script: str, predicted_outcome: str, edge_tier: str, *, abs_delta: float = 0.0) -> list[dict]:
    ...
    # Knockout open/both-score preference: ensure 2-1 / 1-2 / 2-2 exist for directional KO.
    if (
        predicted_outcome != "draw"
        and bool(KNOCKOUT_POLICY.get("prefer_both_score_primary_in_ko", True))
        and game_script in {"low-event", "medium-event", "open-game"}
    ):
        both_score = (
            [{"home": 2, "away": 1}, {"home": 3, "away": 1}, {"home": 2, "away": 0}]
            if predicted_outcome == "home_win"
            else [{"home": 1, "away": 2}, {"home": 1, "away": 3}, {"home": 0, "away": 2}]
        )
        for candidate in both_score:
...
def _split_confidence_fields(
    *,
    ...
    """Fallback split when full multi-factor scorer is not used.

    Exact-score confidence is always stricter than result direction confidence.
    Critical evidence gaps block high score confidence.
    """
    ...
    if result_confidence == "high" and top_probability >= 0.40 and not critical_hard:
        score_confidence = "high"
    elif result_confidence in {"high", "medium"} and top_probability >= 0.30 and not critical_hard:
        score_confidence = "medium"
    else:
        score_confidence = "low"

    if result_confidence == "high" and clean_gap >= 0.16 and top_probability >= 0.34:
        total_goals_confidence = "medium" if top_probability < 0.45 else "high"
    elif result_confidence == "low" or critical_hard:
        total_goals_confidence = "low"
    else:
        total_goals_confidence = "medium" if top_probability >= 0.28 else "low"
...
def predict_match(
    *,
    ...
    # Provisional confidence for knockout builder; final multi-factor score runs after market alignment.
    _prov = _score_prediction_confidence(
        data_home=data_home,
        data_away=data_away,
        abs_edge=abs(float(home_final) - float(away_final)),
        edge_tier=edge_tier,
        evidence_quality=evidence_quality,
        evidence_gaps=evidence_gaps,
        scoreline_distribution=scoreline_distribution,
        dual_track_alignment="untracked",
        market_status=str(market_snapshot.get("status") or "none"),
        predicted_outcome=predicted_outcome,
        phase=str(match.get("phase") or ""),
        three_track_votes=three_track_votes,
    )
    confidence = _prov["confidence"]
    confidence_label = _prov["confidence_label"]
    ...
    avg_data = (data_home + data_away) / 2.0
    # Final multi-factor confidence — earned, not forced by match_id / phase policy.
    _conf = _score_prediction_confidence(
        data_home=data_home,
        data_away=data_away,
        abs_edge=abs(float(home_final) - float(away_final)),
        edge_tier=edge_tier,
    ...
    confidence_label = _conf["confidence_label"]
    confidence_split = {
        "result_confidence": _conf["result_confidence"],
        "score_confidence": _conf["score_confidence"],
        "total_goals_confidence": _conf["total_goals_confidence"],
        "confidence_note": _conf["confidence_note"],
        "confidence_score": _conf["confidence_score"],
        "confidence_factors": _conf["confidence_factors"],
    }
    ...
    if lessons:
        ...
        if total_adj != 0.0:
            ...
            # Re-sync split after lesson adjustment (lessons adjust result level only).
    ...
    return {
        ...
        "prediction": {
            ...
            "confidence_score": confidence_split.get("confidence_score"),
            "confidence_factors": confidence_split.get("confidence_factors"),
            "evidence_quality": evidence_quality,
            "edge_tier": edge_tier,
            "game_script": game_script,
            "three_track_votes": three_track_votes,
