...
- **精准比分命中**: 6 / 72，8.3%
- **总进球命中**: 12 / 72，16.7%
- **淘汰赛更新**: 2026-06-29 南非 0-1 加拿大已写入；当前看板累计评估 73 场，胜平负 49 / 73，67.1%
- **R32 预测已发布**: 2026-06-29 至 2026-07-04 的 16 场 32 强赛均已写入看板
...
- `wiki/public/<edition>/`：官方/既定/可复用事实，以及打包的默认预测基线
- `wiki/person/<edition>/`：用户自己的预测、复盘、运行缓存、手动补充
- 看板展示：`公共事实 + 默认预测 + 用户预测覆盖`
...
- 已存在正式预测时优先复用，不重算
- 开球后进入锁定态，赛果录入后彻底冻结
- 历史重跑与模型实验只写 `backtests/experiment`，不改正式历史
